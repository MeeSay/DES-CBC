...
string Char_to_Bin(string s);
string Dec_to_Bin(int n);
string Hex_to_Bin(string s);
string Bin_to_Hex(string s);
string CBCRandomEnc(string m, string k, string& IV);
string CBCRandomDec(string IV, string c, string k);
string createIV(const int n);
...
public:
	...
	string encrypt(const string& plain_txt, const string& key)
	{
		// making sub-keys
		string key_64 = key; 
		//Char_to_Bin(key);
		...
		string plain_txt_64 = plain_txt;
			//Char_to_Bin(plain_txt); // transforming key from 16-bits hex to 64-bits bin
	...
	string decrypt(const string& plain_txt, const string& key)
	{
		// making sub-keys
		...
		for (int i = 15; i >= 0; i -- )
		{
			keys_56[15 - i] = L_key[i] + R_key[i]; // making 56 bits keys
		}
		for (int i = 0; i < 16; i++)
		{
			key_48[i] = "";
		...
		string plain_txt_64 = plain_txt;
			//Hex_to_Bin(plain_txt); // transforming key from 16-bits hex to 64-bits bin
...
string Char_to_Bin(string s)
{
	...
	for (int i = 0; i < s.size(); i++)
	{
		switch (s[i])
		{
		case '0': bin += "00110000"; break;
		case '1': bin += "00110001"; break;
		case '2': bin += "00110010"; break;
		case '3': bin += "00110011"; break;
		case '4': bin += "00110100"; break;
		case '5': bin += "00110101"; break;
		case '6': bin += "00110110"; break;
		case '7': bin += "00110111"; break;
		case '8': bin += "00111000"; break;
		case '9': bin += "00111001"; break;
		case 'A': bin += "01000001"; break;
		case 'B': bin += "01000010"; break;
		case 'C': bin += "01000011"; break;
		case 'D': bin += "01000100"; break;
		case 'E': bin += "01000101"; break;
		case 'F': bin += "01000110"; break;
		}
	}
	return bin;
}
...
string createIV(const int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		int random_num = rand() % 2; // Tạo ngẫu nhiên giá trị 0 hoặc 1
		IV += to_string(random_num);
	}
	return IV;
}
string xor_add(string s1, string s2)
...
string CBCRandomEnc(string m, string k, string& IV)
{
	string CBCtxt_64 = Char_to_Bin(m); //chuyen plaintxt, key -> bin
	string CBCkey_64 = Char_to_Bin(k);
	if (CBCtxt_64.size() % CBCkey_64.size() != 0 || CBCtxt_64.size() == 0) //so sanh dieu kien
		exit(0);
	else
	{
		int tmp = CBCtxt_64.size() / CBCkey_64.size(); //tmp: m trong ma gia: so doan phaintxt con
		int dem = 0; //chia M ra m doan
		string M[10];
		for (int i = 1; i <= tmp; i++)
		{
			M[i] = "";
			for (int j = 0; j < CBCkey_64.size(); j++)
			{
				M[i] += CBCtxt_64[dem];
				dem++;
			}
		...
		DES_Encryption DES;
		string c[10];
		c[0] = createIV(CBCkey_64.size()); //tao IV
		IV = c[0];
		for (int i = 1; i <= tmp; i++)
		{
			c[i] = Hex_to_Bin(DES.encrypt(xor_add(c[i - 1], M[i]),CBCkey_64));
		}
		string result = "";
		for (int i = 1; i <= tmp; i++)
			result += Bin_to_Hex(c[i]);
		return result;
	}
...
string CBCRandomDec(string IV, string c, string k)
{
	string CBCc_64 = Hex_to_Bin(c);
	string CBCkey_64 = Char_to_Bin(k);
	if(CBCc_64.size() % CBCkey_64.size() != 0 || CBCc_64.size() == 0)
		exit(0);
	else
	{
		int tmp = CBCc_64.size() / CBCkey_64.size(); //tmp: m trong ma gia: so doan phaintxt con
		int dem = 0; //chia M ra m doan
		string C[10];
		for (int i = 1; i <= tmp; i++)
		{
		...
		for (int i = 1; i <= tmp; i++)
		{
			M[i] = Bin_to_Char(xor_add(DES.decrypt(C[i], CBCkey_64),C[i-1]));
		}
		string result = "";
		for (int i = 1; i <= tmp; i++)
			result += M[i];
...
int main()
{
	char m[] = "123456781234567B", * c = NULL, k[] = "12345678";
	string key(k), plaintext(m);
	string IV = "";
	string EN = CBCRandomEnc(plaintext, key, IV);
	for (int i = EN.length(); i > 0; i--) {
		if (i % 2 == 0)
			EN.insert(EN.begin() + i, ' ');
	}
	cout << EN << endl;
	string DE = CBCRandomDec(IV, EN, key);
	cout << DE << endl;
	return 0;
}
